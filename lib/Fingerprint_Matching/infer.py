...
EVAL_MODEL='./lib/Fingerprint_Matching/ridgebase_NoTransform_LP2_NUMPOS3_BS64LR01_10.pt'
device = torch.device("cpu" if torch.cuda.is_available() else "cpu")
# model = Model(device).to(device)
model = Model2(device).to(device)
...
def get_embeddings(segments):
    ...
    for i, segment in enumerate(segments):
        image1 = transform(segment)
        image1 = image1.unsqueeze(0)

        with torch.no_grad():
            image1= image1.to(device)
            x_cl1, _ = model(image1, image1)
            total_cl1.append(x_cl1)

    return total_cl1

def compare_embeddings(emb_1, emb_2):
    all_sims = 0
    sim_list = []
    for i, emb1 in enumerate(emb_1):
        emb2 = emb_2[i]
        with torch.no_grad():
            sim = F.linear(l2_norm(emb1), l2_norm(emb2))
            all_sims += sim
            sim_list.append(sim.item())
            print(sim)
...
def main_full_cl2cl(fingerprint1, fingerprint2):
    sim, sim_list = match_full_cl2cl(fingerprint1, fingerprint2)
    score = sim.item()
    if (score > 0.50):
        pred = "Match Found"
    else:
        pred = "Not a Match"
    return sim.item(), pred, sim_list