...
class SwinModel(nn.Module):
    ...
    def forward(self, x_cl, x_cb):
        ...
        print(x_cl_tokens.shape)
        x_cl_mean = x_cl_tokens.mean(dim=1)
        print(x_cl_mean.shape)
...
class SwinModel_TPS(nn.Module):
    def __init__(self):
        super().__init__()
        self.swin_cl = timm.create_model('swin_base_patch4_window7_224_in22k', pretrained=True, num_classes=0, global_pool='')
        self.swin_cb = timm.create_model('swin_base_patch4_window7_224_in22k', pretrained=True, num_classes=0, global_pool='')
        self.tps     = TPS_SpatialTransformerNetwork(20, (224,224), (224,224), 3)
        self.linear_cl = nn.Sequential(nn.Linear(1024, 1024),
                                       nn.ReLU(),
                                       nn.Linear(1024, 1024))
        # self.linear_cb = nn.Linear(1024, 1024)
...
class GridGenerator(nn.Module):
    ...
    def build_P_prime(self, batch_C_prime):
        ...
        batch_C_prime_with_zeros = torch.cat((batch_C_prime, torch.zeros(
            batch_size, 3, 2).float().to("cuda:0")), dim=1)  # batch_size x F+3 x 2
        batch_T = torch.bmm(batch_inv_delta_C, batch_C_prime_with_zeros)  # batch_size x F+3 x 2
        batch_P_prime = torch.bmm(batch_P_hat, batch_T)  # batch_size x n x 2
        return batch_P_prime  # batch_size x n x 2

class RidgeBaseModel_DA(nn.Module):
    def __init__(self):
        super().__init__()
        self.swin_cl = timm.create_model('swin_base_patch4_window7_224_in22k', pretrained=True, num_classes=0, global_pool='')
        ...
        self.domain_predictor = nn.Sequential(GradientReversal(alpha=0.2),
                                              nn.Linear(1024, 512), 
                                              nn.ReLU(),
                                              nn.Linear(512, 3))

    def get_embeddings(self, image, ftype):
        linear = self.linear_cl if ftype == "contactless" else self.linear_cb
        
        tokens = self.swin_cl(image)
        emb_mean = tokens.mean(dim=1)
        feat = linear(emb_mean)
        tokens = linear(tokens)
        return feat, tokens

    def forward(self, x_cl, x_cb, s_sample):
        x_cl_tokens = self.swin_cl(x_cl)
        x_cl_mean   = x_cl_tokens.mean(dim=1)
        x_cl        = self.linear_cl(x_cl_mean)
        x_cl_tokens = self.linear_cl(x_cl_tokens)
        ...
        x_cb_mean = x_cb_tokens.mean(dim=1)
        x_cb = self.linear_cb(x_cb_mean)
        x_cb_tokens = self.linear_cb(x_cb_tokens)
        
        s_sample_tokens = self.swin_cl(s_sample)
        s_sample_mean = s_sample_tokens.mean(dim=1)
        s_sample_feat = self.linear_cb(s_sample_mean)
        s_sample_tokens = self.linear_cb(s_sample_tokens)
        
        domain_cl_ridgebase = self.domain_predictor(x_cl_mean)
        domain_cb_ridgebase = self.domain_predictor(x_cb_mean)
        domain_synthia      = self.domain_predictor(s_sample_mean)
        
        return x_cl, x_cb, s_sample_feat, x_cl_tokens, x_cb_tokens, s_sample_tokens, (domain_cl_ridgebase, domain_cb_ridgebase, domain_synthia)
...
class HKPolysynthModel_DA(nn.Module):
    def __init__(self):
        super().__init__()
        self.swin_cl = timm.create_model('swin_base_patch4_window7_224_in22k', pretrained=True, num_classes=0, global_pool='')
        self.linear_cl = nn.Linear(1024, 1024)
        self.linear_cb = nn.Linear(1024, 1024)
        self.domain_predictor = nn.Sequential(GradientReversal(alpha=0.2),   # 0.2
                                              nn.Linear(1024, 512),
                                              nn.ReLU(),
                                              nn.Linear(512, 3))
    def get_embeddings(self, image, ftype):
