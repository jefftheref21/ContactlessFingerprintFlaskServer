...
from model import RidgeBaseModel_DA as Model
...
def test_EER(model, device, test_loader, epoch):
    ...
    with open("./misc/cl_fnames_best.txt", "w+") as file:
    ...
    with open("./misc/cb_fnames.txt", "w+") as file:
        for fname in cb_fnames_new:
            file.write(fname + "\n")
    
    scores = F.linear(cl_feats,cb_feats)
    np.save("./misc/task1_cb2cl_score_matrix_best.npy", scores)
    print(scores.shape)
    ...
    with open("./evaluate/cl2cb_fpr_tpr_best_values.json", "w+") as file:
    ...
    plt.title('ROC Curve cl2cb task1')
    plt.legend(loc="lower right")
    plt.savefig("./evaluate/roc_curve_cl2cb_task1_best.png", dpi=300, bbox_inches='tight')
    ...
    np.save("./misc/task1_cl2cl_score_matrix_best.npy", scores)
    ...
    fpr, tpr, thresholds = roc_curve(ids_mod, scores)
    with open("./evaluate/cl2cl_fpr_tpr_values_best.json", "w+") as file:
        json.dump({"fpr": fpr.tolist(), "tpr": tpr.tolist()}, file)
    ...
    plt.legend(loc="lower right")
    plt.savefig("./evaluate/roc_curve_cl2cl_task1_best.png", dpi=300, bbox_inches='tight')
    ...
    with open("./misc/cmc_ranks_cl2cb.txt", "w+") as file:
        for i in range(1, 100):
            file.write(str(i) + " " + str(retmetric.recall_k(k=i)) + "\n")
    ...
    with open("./misc/cmc_ranks_cl2cl.txt", "w+") as file:
        for i in range(1, 100):
            file.write(str(i) + " " + str(retmetric.recall_k(k=i)) + "\n")
...
def main():
    ...
    train_dataset    = Sythia_RidgeBase(args.ccr, split="train")
    val_dataset      = Sythia_RidgeBase(args.ccr, split="test")
    ...
    loss_func = SupConLoss() #DualMSLoss_FineGrained_DA()
